...
// HXW 华芯微 指令集
// ==============================
const HXW_INSTRUCTIONS = new Set([
...
// 伪指令
const HXW_PSEUDO = new Set([
  'MACRO', 'ENDM', 'ORG', 'END', 'EQU', 'DB', 'DW',
  '.ADJUST_IC'
]);

// 预处理条件指令 (支持 #IF / IF / .IF 等多种格式)
const HXW_CONDITIONAL_PSEUDO = new Set([
  'IF', 'IFDEF', 'IFNDEF', 'ELIF', 'ELSE', 'ENDIF'
]);

// #define 类伪指令
const HXW_DEFINE_PSEUDO = new Set([
...
const HXW_IMMEDIATE_INSTRUCTIONS = new Set([
  'MOVIA', 'MOVLW', 'ADDIA', 'ADDLW', 'SUBIA', 'SUBLW',
  'ANDIA', 'ANDLW', 'IORIA', 'IORLW', 'XORIA', 'XORLW',
  'RETIA', 'RETLW'
]);

// RAM 地址指令（第一个操作数为 RAM 地址，需要检查是否定义）
const HXW_RAM_INSTRUCTIONS = new Set([
  'XORAR', 'XORWF', 'XORR', 'XORF',
  'ADDAR', 'ADDWF', 'ADCAR', 'ADCWF', 'ADCR', 'ADCF',
  'SUBAR', 'SUBWF', 'SBCAR', 'SBCWF', 'SBCR', 'SBCF',
  'ANDAR', 'ANDWF', 'ANDR', 'ANDF',
  'IORAR', 'IORWF', 'IORR', 'IORF',
  'COMR', 'COMF',
  'DECR', 'DECF', 'DECRSZ', 'DECFSZ',
  'INCR', 'INCF', 'INCRSZ', 'INCFSZ',
  'MOVAR', 'MOVWF', 'MOVR', 'MOVF',
  'CLRR', 'CLRF',
  'RLR', 'RLF', 'RRR', 'RRF',
  'SWAPR', 'SWAPF',
  'BCR', 'BCF', 'BSR', 'BSF',
  'BTRSC', 'BTFSC', 'BTRSS', 'BTFSS'
]);

// 操作数规则
const HXW_RULES: { [key: string]: { operands: number } } = {
  'BCR': { operands: 2 }, 'BCF': { operands: 2 },
  'BSR': { operands: 2 }, 'BSF': { operands: 2 },
  'BTRSC': { operands: 2 }, 'BTFSC': { operands: 2 },
  'BTRSS': { operands: 2 }, 'BTFSS': { operands: 2 },
  'MOVR': { operands: 2 }, 'MOVF': { operands: 2 },
  'DECR': { operands: 2 }, 'DECF': { operands: 2 },
  'DECRSZ': { operands: 2 }, 'DECFSZ': { operands: 2 },
  'INCR': { operands: 2 }, 'INCF': { operands: 2 },
  'INCRSZ': { operands: 2 }, 'INCFSZ': { operands: 2 },
  'ADDAR': { operands: 2 }, 'ADDWF': { operands: 2 },
  'ADCAR': { operands: 2 }, 'ADCWF': { operands: 2 },
  'SUBAR': { operands: 2 }, 'SUBWF': { operands: 2 },
  'SBCAR': { operands: 2 }, 'SBCWF': { operands: 2 },
  'ANDAR': { operands: 2 }, 'ANDWF': { operands: 2 },
  'IORAR': { operands: 2 }, 'IORWF': { operands: 2 },
  'XORAR': { operands: 2 }, 'XORWF': { operands: 2 },
  'COMR': { operands: 2 }, 'COMF': { operands: 2 },
  'RLR': { operands: 2 }, 'RLF': { operands: 2 },
  'RRR': { operands: 2 }, 'RRF': { operands: 2 },
  'SWAPR': { operands: 2 }, 'SWAPF': { operands: 2 },
  'CLRR': { operands: 1 }, 'CLRF': { operands: 1 },
  'MOVAR': { operands: 1 }, 'MOVWF': { operands: 1 },
  'MOVIA': { operands: 1 }, 'MOVLW': { operands: 1 },
  'ADDIA': { operands: 1 }, 'ADDLW': { operands: 1 },
  'SUBIA': { operands: 1 }, 'SUBLW': { operands: 1 },
  'ANDIA': { operands: 1 }, 'ANDLW': { operands: 1 },
  'IORIA': { operands: 1 }, 'IORLW': { operands: 1 },
  'XORIA': { operands: 1 }, 'XORLW': { operands: 1 },
  'RETIA': { operands: 1 }, 'RETLW': { operands: 1 },
  'CALL': { operands: 1 }, 'GOTO': { operands: 1 },
  'NOP': { operands: 0 }, 'CLRWDT': { operands: 0 }, 'SLEEP': { operands: 0 },
  'DAA': { operands: 0 }, 'DAW': { operands: 0 }, 'DAS': { operands: 0 },
  'RETURN': { operands: 0 }, 'RETFIE': { operands: 0 },
  'CLRA': { operands: 0 }, 'CLRW': { operands: 0 },
};
...
function collectDefinesFromFile(filePath: string, visited: Set<string>): Set<string> {
  ...
  for (const line of lines) {
    ...
    if (includeMatch) {
      const incPath = path.join(dir, includeMatch[2]);
      console.log(`HXW: Found include: ${includeMatch[2]} -> resolved: ${incPath}, exists: ${fs.existsSync(incPath)}`);
  ...
  for (const incPath of includePaths) {
    console.log(`HXW: Processing include file: ${incPath}, exists: ${fs.existsSync(incPath)}`);
    const includeDefines = collectDefinesFromFile(incPath, visited);
    console.log(`HXW: Include ${incPath} defines: ${Array.from(includeDefines).join(', ')}`);
  ...
  for (const line of lines) {
    ...
    if (active) {
      ...
      if (defineMatch) {
        ...
        console.log(`HXW collectDefinesFromFile - Collected: ${defineName} from ${filePath}`);
      }
    }
  }
  
  console.log(`HXW collectDefinesFromFile - Final defines: ${Array.from(defines).join(', ')}`);
  return defines;
}
...
function getInactiveLinesHXW(content: string, filePath: string): Set<number> {
  console.log(`HXW: getInactiveLinesHXW called for ${filePath}`);
  const lines = content.split('\n');
  ...
  includeDefines.forEach(d => defines.add(d));
  console.log(`HXW getInactiveLinesHXW - Defines collected: ${Array.from(defines).join(', ')}`);
...
export function checkHXWSyntax(document: vscode.TextDocument): vscode.Diagnostic[] {
  ...
  const inactiveLines = getInactiveLinesHXW(content, document.uri.fsPath);
  ...
  for (let line = 0; line < document.lineCount; line++) {
    ...
    // 伪指令不报错（包括条件编译指令）
    if (HXW_PSEUDO.has(inst) || HXW_DEFINE_PSEUDO.has(inst) || HXW_CONDITIONAL_PSEUDO.has(inst)) {
      continue;
    }
    ...
    if (rule && !macroLines.has(line)) {
      ...
      if (rule.operands > 0) {
        ...
        // RAM 地址指令（*AR, *WF, *R, *F 等）验证第一个操作数
        // 立即数指令验证第一个操作数
        // 其他双操作数指令验证第二个操作数
        let operandIdx = -1;
        if (HXW_RAM_INSTRUCTIONS.has(inst) || HXW_IMMEDIATE_INSTRUCTIONS.has(inst)) {
          // 这些指令验证第一个操作数
          if (actualOperandCount >= 1) {
            operandIdx = 0;
          }
        } else if (actualOperandCount >= 2) {
          // 其他双操作数指令验证第二个操作数
          operandIdx = 1;
        }
